...
class PartnerScraper(BaseScraper):
    """Simple partner listing scraper.

    PoC accepts a `selectors` dict with keys: `name`, `website`, `desc`, and
    optionally `contact`.
    """
    ...
    def parse(self, html: str, selectors: Dict[str, str]) -> Dict[str, Any]:
        ...
        return {
            "name": _select("name"),
            "website": _select("website"),
            "description": _select("desc"),
            "contact": _select("contact"),
        }
